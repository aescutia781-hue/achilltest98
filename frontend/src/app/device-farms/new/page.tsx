...
export default function NewDeviceFarmPage() {
  ...
  async function save() {
    ...
    setSaving(true)
    try {
      await api.post('/api/device-farms', {
        name: name.trim(),
  ...
  if (loading || loadingDevices) return <Loading/>
      ...
      <div style={{ maxWidth:'1400px', margin:'0 auto', padding:'2rem 1.5rem' }}>
        <div style={{ marginBottom:'1.5rem' }}>
          ...
          <p style={{ color:'#7070a0', fontSize:'.9375rem' }}>
            Selecciona hasta {MAX} dispositivos. Tus suites se ejecutarán en paralelo en todos.
          </p>
        </div>
            ...
            <div style={cardStyle}>
              <Field label="Nombre de la farm">
                <input
                  ...
                  placeholder="Ej. Top Mobile QA Set"
                  style={inputStyle}
                />
              </Field>
                ...
                <div key={cat} style={{ marginBottom:'1rem' }}>
                  ...
                  <div style={{
                    ...
                    {list.map(d => {
                      ...
                      return (
                        <button
                          key={d.id} onClick={() => toggleDevice(d.id)}
                          disabled={isDisabled}
                          style={{
                            textAlign:'left',
            ...
            <div style={cardStyle}>
              ...
              {selected.length === 0 ? (
                <div style={{
                  ...
                  color:'#5a5a7a', fontSize:'.8125rem',
                }}>
                  <div style={{ fontSize:'2rem', marginBottom:'.5rem' }}>🏭</div>
                  Selecciona dispositivos para verlos aquí
                </div>
              ) : (
                <div style={{
                  display:'flex', flexWrap:'wrap', gap:'.75rem',
                  justifyContent:'center', maxHeight:'500px', overflowY:'auto',
                  padding:'.5rem',
                }}>
                  {selectedDevices.map(d => (
                    <div key={d.id} style={{ textAlign:'center', position:'relative' }}>
                      ...
                      >
                        <div style={{
                          width:'100%', height:'100%', background:'#1a1a2a',
                          display:'flex', alignItems:'center', justifyContent:'center',
                          fontSize:'.6rem', color:'#5a5a7a',
                        }}>
                          {d.name.split(' ').slice(-1)[0]}
                        </div>
                      </DeviceFrame>
                      <div style={{
                        fontSize:'.6rem', color:'#7070a0',
                        marginTop:'.25rem', maxWidth:80,
              ...
              <div style={{ marginTop:'1rem', display:'flex', gap:'.5rem' }}>
                <button onClick={() => router.push('/device-farms')} style={btnGhostStyle}>
                  Cancelar
                </button>
                <button onClick={save} disabled={saving} style={{
                  ...btnPrimaryStyle, flex:1,
                  opacity: saving ? .6 : 1, cursor: saving ? 'not-allowed' : 'pointer',
                }}>
                  {saving ? 'Guardando...' : '💾 Guardar farm'}
                </button>
              </div>
            </div>
          </div>
